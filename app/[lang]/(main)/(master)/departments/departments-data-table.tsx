...
export function DepartmentsDataTable({ 
  ...
  const columns: ColumnDef<Department>[] = [
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        if (typeof department.company === 'string') {
          return <span className="text-sm">{department.company}</span>;
        } else if (department.company) {
          return (
            <div className="flex flex-col">
              <span className="font-medium text-sm">{department.company.name}</span>
              <span className="text-xs text-muted-foreground">{department.company.code} • ID: {department.company.id}</span>
            </div>
          );
        } else if (department.company_id) {
          return <Badge variant="outline" color="secondary">ID: {department.company_id}</Badge>;
        } else {
    ...
    },
    {
      ...
      cell: ({ row }) => {
        const department = row.original;
        if (typeof department.division === 'string') {
          return <span className="text-sm">{department.division}</span>;
        } else if (department.division) {
          return (
            <div className="flex flex-col">
              <span className="font-medium text-sm">{department.division.name}</span>
              {department.division.code && (<span className="text-xs text-muted-foreground">{department.division.code}</span>)}
            </div>
          );
        } else if (department.division_id) {
          return <Badge variant="outline" color="secondary">ID: {department.division_id}</Badge>;
        } else {
          return <span className="text-muted-foreground">—</span>;
        }
      },
    },
    {
      ...
      header: "Status",
      cell: ({ row }) => {
        const isLoading = togglingDepartmentIds.has(row.original.id);
  ...
  const handleToggleStatus = async (id: number, currentStatus: any) => {
    // Prevent multiple clicks on the same department
    if (togglingDepartmentIds.has(id) || toggleStatusMutation.isPending) return;
    ...
    try {
      ...
      toast.success(`Department status updated to ${current === 1 ? "Inactive" : "Active"}`);
    } catch (error: any) {
      toast.error(error?.response?.data?.message || "Failed to update department status. Please try again.");
    } finally {
